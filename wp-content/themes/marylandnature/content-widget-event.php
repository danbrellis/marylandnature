...
// extra event classes
$classes = apply_filters( 'em_widget_event_classes', ['eventStamp'] );
?>
...
<li id="post-<?php the_ID(); ?>" <?php post_class( $classes ); ?>>
	...
	if ( apply_filters( 'em_show_widget_event_title', true ) ) :
        ...
        <h3 class="eventStamp__title"><a href="<?php the_permalink(); ?>" class="eventStamp__link"><?php the_title(); ?></a></h3>
    ...
    <div class="eventStamp__details">
        <time class="eventStamp__date">
            <?php echo nhsm_format_date_range(strtotime($post->event_occurrence_start_date), strtotime($post->event_occurrence_end_date), em_is_all_day($post->ID)); ?>
        </time>
        <address class="eventStamp__location">
            <?php
